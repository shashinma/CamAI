'''
cv2 Python binary extension loader
'''
import os
import importlib
import sys
...
try:
    ...
    print('cv2 bindings requires "numpy" package.')
...
def bootstrap():
    ...
    if hasattr(sys, 'OpenCV_LOADER'):
        print(sys.path)
        raise ImportError('ERROR: recursion is detected during loading of "cv2" binary extensions. Check cv2 installation.')
    ...
    import platform
    if DEBUG: print('cv2 loader: os.name="{}"  platform.system()="{}"'.format(os.name, str(platform.system())))
    ...
    def load_first_config(fnames, required=True):
        for fname in fnames:
            ...
            if not os.path.exists(fpath):
                if DEBUG: print('cv2 loader: config not found, skip: {}'.format(fpath))
                continue
            if DEBUG: print('cv2 loader: loading config: {}'.format(fpath))
            exec_file_wrapper(fpath, g_vars, l_vars)
            return True
        if required:
            raise ImportError('cv2 loader: missing configuration file: {}. Check cv2 installation.'.format(fnames))
    ...
    if DEBUG: print('cv2 loader: PYTHON_EXTENSIONS_PATHS={}'.format(str(l_vars['PYTHON_EXTENSIONS_PATHS'])))
    if DEBUG: print('cv2 loader: BINARIES_PATHS={}'.format(str(l_vars['BINARIES_PATHS'])))
    ...
    if hasattr(sys, 'OpenCV_REPLACE_SYS_PATH_0'):
        ...
        try:
            ...
            if DEBUG: print('cv2 loader: exception during checking workaround for sys.path[0]')
    ...
    if os.name == 'nt':
        ...
        if DEBUG: print('cv2 loader: PATH={}'.format(str(os.environ['PATH'])))
    else:
